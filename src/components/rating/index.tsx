...
function calculateCurrentPosition(totalIcons: number, positionX: number, width: number) {
	...
	for (let i = 0; i < totalIcons; i += 1) {
		// si la posición es inferior al ícono de un cuarto.
...
const Rating = ({
	...
	// Vuelve a renderizar cuando cambia a ratingValue 
	React.useEffect(() => dispatch({ type: "MouseClick", payload: ratingValue }), [ratingValue]);

	/**
	 * use el evento del puntero en lugar del evento del mouse
	 *
	 * @param event
	...
	const onPointerMove = (event: React.PointerEvent<HTMLSpanElement>) => {
		...
		// obtener la posición y el ancho del elemento del tramo principal
		const { left, width } = currentTarget.children[0].getBoundingClientRect();
		const positionX = clientX - left;

		// Obtenga la posición actual del puntero mientras se mueve sobre los íconos
		const currentValue = calculateCurrentPosition(totalIcons, positionX, width);

		// establecer el valor para indicar
	...
	const onPointerEnter = (event: React.PointerEvent<HTMLSpanElement>) => {
		// habilitar solo en dispositivos táctiles
		if (!isTouchDevice()) return;

		// llamar para saber el valor
		onPointerMove(event);
	};
	...
	const onRate = () => {
		if (hoverValue) {
			...
			// actualizar valor al hacer clic
	...
	// si hay un valor de calificación local, conviértalo en porcentaje
	...
	/**
	 * convertir valor de calificación a valor porcentual
	...
	// convertir valor a valor de representación
